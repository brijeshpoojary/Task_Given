...
case when ans.status=-99999 then '99' else case when ans.status=-1 then '0' else case when ans."answerGrade" is null then '0' else cast(ans."answerGrade" as text) end end end as answerscore 
from 
...
and s.id in(30963,30960,33458,30952,30664,30646,30225,31062,30377,33823)
and s.id=sg.institution_id
and sg.id=sasa.student_group_id
and sasa.assessment_id=assess.id
and assess.id in(125,126)
