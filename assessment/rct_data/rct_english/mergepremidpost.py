...
def main():
  ...
  for row in data:
    ...
    students[studentid]["mid"]={}
    students[studentid]["mid"]["details"]=""
    for count in range(11,len(row)):
      students[studentid]["mid"]["details"]=students[studentid]["mid"]["details"]+"|"+row[count]
  ...
  outputfile.write("BLK|CLUST|SCODE|SNAME|STUID|CNAME|CSEX|DOB|CLANG|CLASS|PROGNAME")
  for e in range (0,len(edetails["pre"])):
    outputfile.write("|"+edetails["pre"][e])
  for e in range (0,len(edetails["mid"])):
    outputfile.write("|"+edetails["mid"][e])
